...
public class FileUpload {
    ...
    public static int putFile(String url, byte[] bytes) throws IOException{
        ...
        for (Header header : headers)
            System.out.println(TAG + " " + header.toString());
        ...
        System.out.println(TAG + " " + "response: " + response.getStatusLine());
        HttpEntity responseEntity = response.getEntity();

        if (null != responseEntity)
            System.out.println(TAG + " " + Get.convertStreamToString(responseEntity.getContent()));
